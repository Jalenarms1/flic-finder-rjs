...
export default function MovieModal(props) {
    ...
    // const loadNewRandom = () => {
    //   setRandomMovie(props.movieTitles[Math.floor(Math.random() * props.movieTitles.length)])
    ...
    return (
        <Modal className="fade" show={props.show} onHide={props.handleClose}>
            ...
            <Modal.Body className="bg-dark">
              <div className="w-100 d-flex justify-content-center">
                <div className="card const-shadow m-1" style={{width: '22rem'}}>
                    <img src={props.randomMovie.Poster} className="card-img-top" alt="..." style={{height: '22rem'}}/>
                    <div className="card-body card-body-pos bg-dark">
                        <div className="wrap-movie-info text-light">
                          <h5 className="card-title">{props.randomMovie.Title}</h5>
                          <p className="card-text">Directed by: {props.randomMovie.Director}</p>
                          <p>{props.randomMovie.Plot}</p>
                        </div>
                    </div>
                    <div className="card-footer">
                      <a className="btn btn-sm btn-outline-dark" href={`https://www.imdb.com/title/${props.randomMovie.imdbID}`} rel="noreferrer" target="_blank">View on IMDb</a>
                    </div>
                </div>
            ...
            <Modal.Footer className="bg-dark">
              <Button variant="primary" onClick={props.getRandomMovie}>Redo</Button>
